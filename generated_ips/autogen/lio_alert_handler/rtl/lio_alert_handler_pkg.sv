...
package lio_alert_handler_pkg;

  // these localparams are generated based on the system top-level configuration
  localparam int unsigned      NAlerts   = lio_alert_handler_reg_pkg::NAlerts;   // maximum 252
  localparam int unsigned      EscCntDw  = lio_alert_handler_reg_pkg::EscCntDw;  // maximum 32
  localparam int unsigned      AccuCntDw = lio_alert_handler_reg_pkg::AccuCntDw; // maximum 32
  localparam int unsigned      NLpg      = lio_alert_handler_reg_pkg::NLpg;
  localparam int unsigned      NLpgWidth = lio_alert_handler_reg_pkg::NLpgWidth;
  localparam logic [NAlerts-1:0][NLpgWidth-1:0] LpgMap = lio_alert_handler_reg_pkg::LpgMap;
  // enable async transitions for specific RX/TX pairs
  localparam bit [NAlerts-1:0] AsyncOn   = lio_alert_handler_reg_pkg::AsyncOn;

  // common constants, do not change
  localparam int unsigned N_CLASSES   = lio_alert_handler_reg_pkg::N_CLASSES;
  localparam int unsigned N_ESC_SEV   = lio_alert_handler_reg_pkg::N_ESC_SEV;
  localparam int unsigned N_PHASES    = lio_alert_handler_reg_pkg::N_PHASES;
  localparam int unsigned N_LOC_ALERT = lio_alert_handler_reg_pkg::N_LOC_ALERT;

  localparam int unsigned PING_CNT_DW = lio_alert_handler_reg_pkg::PING_CNT_DW;
  localparam int unsigned PHASE_DW    = lio_alert_handler_reg_pkg::PHASE_DW;
  localparam int unsigned CLASS_DW    = lio_alert_handler_reg_pkg::CLASS_DW;
...
endpackage : lio_alert_handler_pkg
