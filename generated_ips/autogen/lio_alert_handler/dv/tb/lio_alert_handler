...
module tb;
  ...
  import uvm_pkg::*;
  import dv_utils_pkg::*;
  import lio_alert_handler_env_pkg::*;
  import lio_alert_handler_test_pkg::*;
  ...
  lio_alert_handler_if lio_alert_handler_if(.clk(clk), .rst_n(rst_n));
  ...
  for (genvar k = 0; k < NUM_ALERTS; k++) begin : gen_alert_if
    ...
    assign lio_alert_handler_if.alert_ping_reqs[k] = dut.gen_alerts[k].u_alert_receiver.ping_req_i;
    initial begin
  ...
  for (genvar k = 0; k < NUM_ESCS; k++) begin : gen_esc_if
    ...
    assign lio_alert_handler_if.esc_ping_reqs[k] = dut.gen_esc_sev[k].u_esc_sender.ping_req_i;
    initial begin
  ...
  lio_alert_handler dut (
    ...
    .lpg_cg_en_i          ( lio_alert_handler_if.lpg_cg_en  ),
    .lpg_rst_en_i         ( lio_alert_handler_if.lpg_rst_en ),
  ...
  initial begin
    ...
    uvm_config_db#(virtual lio_alert_handler_if)::set(null, "*.env", "lio_alert_handler_vif",
                   lio_alert_handler_if);
