...
class lio_alert_handler_env extends cip_base_env #(
    .CFG_T              (lio_alert_handler_env_cfg),
    .COV_T              (lio_alert_handler_env_cov),
    .VIRTUAL_SEQUENCER_T(lio_alert_handler_virtual_sequencer),
    .SCOREBOARD_T       (lio_alert_handler_scoreboard)
  );
  `uvm_component_utils(lio_alert_handler_env)
  ...
  function void build_phase(uvm_phase phase);
    ...
    if (!uvm_config_db#(lio_alert_handler_vif)::get(this, "", "lio_alert_handler_vif",
                                                cfg.lio_alert_handler_vif)) begin
      `uvm_fatal(`gfn, "failed to get lio_alert_handler_vif from uvm_config_db")
    end
  endfunction
