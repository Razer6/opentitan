...
class lio_alert_handler_scoreboard extends cip_base_scoreboard #(
    .CFG_T(lio_alert_handler_env_cfg),
    .RAL_T(lio_alert_handler_reg_block),
    .COV_T(lio_alert_handler_env_cov)
  );
  `uvm_component_utils(lio_alert_handler_scoreboard)
  ...
  virtual task process_alert_fifo();
    foreach (alert_fifo[i]) begin
      ...
      automatic int lpg_index = lio_alert_handler_reg_pkg::LpgMap[index];
      fork
        forever begin
          ...
          alert_en = ral.alert_en_shadowed[index].get_mirrored_value() &&
              prim_mubi_pkg::mubi4_test_false_loose(cfg.lio_alert_handler_vif.lpg_cg_en[lpg_index]) &&
              prim_mubi_pkg::mubi4_test_false_loose(cfg.lio_alert_handler_vif.lpg_rst_en[lpg_index]);
  ...
  virtual task check_ping_triggered_cycles();
    ...
    if (cfg.lio_alert_handler_vif.alert_ping_reqs > 0) begin
      if (cfg.en_cov) begin
        int alert_id = $clog2(cfg.lio_alert_handler_vif.alert_ping_reqs);
        cov.ping_with_lpg_cg_wrap[alert_id].alert_ping_with_lpg_cg.sample(
            cfg.alert_host_cfg[alert_id].en_alert_lpg);
      end
      break;
    end
    if (cfg.lio_alert_handler_vif.esc_ping_reqs > 0) break;
      cfg.clk_rst_vif.wait_clks(1);
      ping_wait_cycs++;
    end
    ...
    wait (cfg.lio_alert_handler_vif.alert_ping_reqs == 0 && cfg.lio_alert_handler_vif.esc_ping_reqs == 0);
  endtask
  ...
  virtual task check_crashdump();
    forever begin
      ...
      @(cfg.crashdump_vif.pins) begin
        lio_alert_handler_pkg::alert_crashdump_t crashdump_val =
            lio_alert_handler_pkg::alert_crashdump_t'(cfg.crashdump_vif.sample());
