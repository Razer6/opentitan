...
// combine all lio_alert_handler seqs (except below seqs) in one seq to run sequentially
// 1. csr seq, which requires scb to be disabled
class lio_alert_handler_stress_all_vseq extends lio_alert_handler_base_vseq;
  `uvm_object_utils(lio_alert_handler_stress_all_vseq)

  `uvm_object_new

  task body();
    bit entropy_test_flag; // this flag ensures entropy test only runs once due to its long runtime
    string seq_names[] = {"lio_alert_handler_smoke_vseq",
                          "lio_alert_handler_random_alerts_vseq",
                          "lio_alert_handler_random_classes_vseq",
                          "lio_alert_handler_esc_intr_timeout_vseq",
                          "lio_alert_handler_esc_alert_accum_vseq",
                          "lio_alert_handler_sig_int_fail_vseq",
                          "lio_alert_handler_entropy_vseq"};
    for (int i = 1; i <= num_trans; i++) begin
      uvm_sequence            seq;
      lio_alert_handler_base_vseq alert_vseq;
      uint seq_idx = entropy_test_flag ? $urandom_range(0, seq_names.size - 2) :
                                         $urandom_range(0, seq_names.size - 1);
      if (seq_names[seq_idx] == "lio_alert_handler_entropy_vseq") entropy_test_flag = 1;
      ...
      if (seq_names[seq_idx] == "alert_common_vseq") begin
        lio_alert_handler_common_vseq common_vseq;
