...
class lio_alert_handler_base_vseq extends cip_base_vseq #(
    .CFG_T               (lio_alert_handler_env_cfg),
    .RAL_T               (lio_alert_handler_reg_block),
    .COV_T               (lio_alert_handler_env_cov),
    .VIRTUAL_SEQUENCER_T (lio_alert_handler_virtual_sequencer)
  );
  `uvm_object_utils(lio_alert_handler_base_vseq)
  ...
  virtual task check_alert_interrupts();
    ...
    `DV_WAIT((cfg.lio_alert_handler_vif.alert_ping_reqs || cfg.lio_alert_handler_vif.esc_ping_reqs) == 0)
  ...
  virtual function void set_alert_lpg(int alert_i);
    int       lpg_i = lio_alert_handler_reg_pkg::LpgMap[alert_i];
    bit [1:0] set_lpg;

    if (cfg.lio_alert_handler_vif.get_lpg_status(lpg_i) == 0) begin
      `DV_CHECK_STD_RANDOMIZE_WITH_FATAL(set_lpg, set_lpg > 0;);
      if (set_lpg[0]) cfg.lio_alert_handler_vif.set_lpg_cg_en(lpg_i);
      if (set_lpg[1]) cfg.lio_alert_handler_vif.set_lpg_rst_en(lpg_i);
      foreach (lio_alert_handler_reg_pkg::LpgMap[i]) begin
        if (lio_alert_handler_reg_pkg::LpgMap[i] == lpg_i) cfg.alert_host_cfg[i].en_alert_lpg = 1;
      end
    end
  endfunction
...
endclass : lio_alert_handler_base_vseq
