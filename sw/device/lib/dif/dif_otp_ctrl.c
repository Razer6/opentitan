...
dif_result_t dif_otp_ctrl_get_err_code(const dif_otp_ctrl_t *otp,
                                       uint32_t partition,
                                       dif_otp_ctrl_error_t *err) {
  if (otp == NULL || err == NULL ||
      partition >
          (OTP_CTRL_PARAM_NUM_PART + kDifOtpCtrlStatusCodeHasCauseLast)) {
    return kDifBadArg;
  }
  ...
  ptrdiff_t address =
      OTP_CTRL_ERR_CODE_0_REG_OFFSET + partition * (ptrdiff_t)sizeof(uint32_t);
...
dif_result_t dif_otp_ctrl_get_status(const dif_otp_ctrl_t *otp,
  ...
  static const bitfield_bit32_index_t kPartitionIndices[] = {
    [kDifOtpCtrlPartitionStatusCodeVendorTestError] =
        OTP_CTRL_PARTITION_STATUS_0_VENDOR_TEST_ERROR_BIT,
    [kDifOtpCtrlPartitionStatusCodeCreatorSwCfgError] =
        OTP_CTRL_PARTITION_STATUS_0_CREATOR_SW_CFG_ERROR_BIT,
    [kDifOtpCtrlPartitionStatusCodeOwnerSwCfgError] =
        OTP_CTRL_PARTITION_STATUS_0_OWNER_SW_CFG_ERROR_BIT,
#if defined(OPENTITAN_IS_EARLGREY)
    [kDifOtpCtrlPartitionStatusCodeRotCreatorAuthCodesignError] =
        OTP_CTRL_PARTITION_STATUS_0_ROT_CREATOR_AUTH_CODESIGN_ERROR_BIT,
    [kDifOtpCtrlPartitionStatusCodeRotCreatorAuthStateError] =
        OTP_CTRL_PARTITION_STATUS_0_ROT_CREATOR_AUTH_STATE_ERROR_BIT,
#elif defined(OPENTITAN_IS_DARJEELING)
    [kDifOtpCtrlPartitionStatusCodeOwnershipSlotStateError] =
        OTP_CTRL_PARTITION_STATUS_0_OWNERSHIP_SLOT_STATE_ERROR_BIT,
    [kDifOtpCtrlPartitionStatusCodeRotCreatorAuthError] =
        OTP_CTRL_PARTITION_STATUS_0_ROT_CREATOR_AUTH_ERROR_BIT,
    [kDifOtpCtrlPartitionStatusCodeRotOwnerAuthSlot0Error] =
        OTP_CTRL_PARTITION_STATUS_0_ROT_OWNER_AUTH_SLOT0_ERROR_BIT,
    [kDifOtpCtrlPartitionStatusCodeRotOwnerAuthSlot1Error] =
        OTP_CTRL_PARTITION_STATUS_0_ROT_OWNER_AUTH_SLOT1_ERROR_BIT,
    [kDifOtpCtrlPartitionStatusCodePlatIntegAuthSlot0Error] =
        OTP_CTRL_PARTITION_STATUS_0_PLAT_INTEG_AUTH_SLOT0_ERROR_BIT,
    [kDifOtpCtrlPartitionStatusCodePlatIntegAuthSlot1Error] =
        OTP_CTRL_PARTITION_STATUS_0_PLAT_INTEG_AUTH_SLOT1_ERROR_BIT,
    [kDifOtpCtrlPartitionStatusCodePlatOwnerAuthSlot0Error] =
        OTP_CTRL_PARTITION_STATUS_0_PLAT_OWNER_AUTH_SLOT0_ERROR_BIT,
    [kDifOtpCtrlPartitionStatusCodePlatOwnerAuthSlot1Error] =
        OTP_CTRL_PARTITION_STATUS_0_PLAT_OWNER_AUTH_SLOT1_ERROR_BIT,
    [kDifOtpCtrlPartitionStatusCodePlatOwnerAuthSlot2Error] =
        OTP_CTRL_PARTITION_STATUS_0_PLAT_OWNER_AUTH_SLOT2_ERROR_BIT,
    [kDifOtpCtrlPartitionStatusCodePlatOwnerAuthSlot3Error] =
        OTP_CTRL_PARTITION_STATUS_0_PLAT_OWNER_AUTH_SLOT3_ERROR_BIT,
    [kDifOtpCtrlPartitionStatusCodeExtNvmError] =
        OTP_CTRL_PARTITION_STATUS_0_EXT_NVM_ERROR_BIT,
    [kDifOtpCtrlPartitionStatusCodeRomPatchError] =
        OTP_CTRL_PARTITION_STATUS_0_ROM_PATCH_ERROR_BIT,
#else
#error "dif_otp_ctrl does not support this top"
#endif
    [kDifOtpCtrlPartitionStatusCodeHwCfg0Error] =
        OTP_CTRL_PARTITION_STATUS_0_HW_CFG0_ERROR_BIT,
    [kDifOtpCtrlPartitionStatusCodeHwCfg1Error] =
        OTP_CTRL_PARTITION_STATUS_0_HW_CFG1_ERROR_BIT,
    [kDifOtpCtrlPartitionStatusCodeSecret0Error] =
        OTP_CTRL_PARTITION_STATUS_0_SECRET0_ERROR_BIT,
    [kDifOtpCtrlPartitionStatusCodeSecret1Error] =
        OTP_CTRL_PARTITION_STATUS_0_SECRET1_ERROR_BIT,
    [kDifOtpCtrlPartitionStatusCodeSecret2Error] =
        OTP_CTRL_PARTITION_STATUS_0_SECRET2_ERROR_BIT,
#if defined(OPENTITAN_IS_DARJEELING)
    [kDifOtpCtrlPartitionStatusCodeSecret3Error] =
        OTP_CTRL_PARTITION_STATUS_0_SECRET3_ERROR_BIT,
...
#error "dif_otp_ctrl does not support this top"
    ...
    [kDifOtpCtrlPartitionStatusCodeLifeCycleError] =
        OTP_CTRL_PARTITION_STATUS_0_LIFE_CYCLE_ERROR_BIT,
  };

  static const bitfield_bit32_index_t kStatusIndices[] = {
  ...
  if (!bitfield_bit32_read(status_code, OTP_CTRL_STATUS_PARTITION_ERROR_BIT)) {
    // If there is any partition error, we need to read the corresponding
    // partition status register
    uint32_t partition_status = mmio_region_read32(
        otp->base_addr, OTP_CTRL_PARTITION_STATUS_0_REG_OFFSET);

    for (uint32_t i = 0; i < ARRAYSIZE(kPartitionIndices); ++i) {
      // If the error is not present at all, we clear its cause bit if relevant,
      // and bail immediately.
      if (!bitfield_bit32_read(partition_status, kPartitionIndices[i])) {
      ...
      DIF_RETURN_IF_ERROR(
          dif_otp_ctrl_get_err_code(otp, i, &status->causes[i]));
    }
  }

  for (uint32_t i = kDifOtpCtrlStatusCodeHasCauseFirst;
       i < ARRAYSIZE(kStatusIndices); ++i) {
    // If the error is not present at all, we clear its cause bit if relevant,
    // and bail immediately.
    if (!bitfield_bit32_read(status_code, kStatusIndices[i])) {
      if (i <= kDifOtpCtrlStatusCodeHasCauseLast) {
        status->causes[OTP_CTRL_PARAM_NUM_PART + i -
                       kDifOtpCtrlStatusCodeHasCauseFirst] = kDifOtpCtrlErrorOk;
      }
      continue;
    }

    status->codes =
        bitfield_bit32_write(status->codes,
                             (bitfield_bit32_index_t)OTP_CTRL_PARAM_NUM_PART +
                                 i - kDifOtpCtrlStatusCodeHasCauseFirst,
                             true);

    if (i <= kDifOtpCtrlStatusCodeHasCauseLast) {
      DIF_RETURN_IF_ERROR(dif_otp_ctrl_get_err_code(
          otp, OTP_CTRL_PARAM_NUM_PART + i - kDifOtpCtrlStatusCodeHasCauseFirst,
          &status->causes[OTP_CTRL_PARAM_NUM_PART + i -
                          kDifOtpCtrlStatusCodeHasCauseFirst]));
    }
  }
