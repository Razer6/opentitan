...
static rom_error_t second_rom_boot(void) {
  ...
  // TODO Configure ePMP for the ROM patch region
  // base_rom_epmp_unlock_second_rom_patch_ram(patch);
 ...
 * $ ./util/design/sparse-fsm-encode.py -d 6 -m 2 -n 16 \
 *     -s 519644925 --language=c
 */
// clang-format off
#define ROM_STATES(X)                                                               \
  X(kRomStateInit,           0x5616, base_rom_state_init, NULL)                          \
  X(kRomStateBootSecondRom,  0x0a92, base_rom_state_boot_second_rom, NULL)
// clang-format on
