...
#(
  ...
  rstmgr_leaf_rst #(
    ...
    .scan_rst_ni,
    .scanmode_i,
    .rst_en_o(rst_en_o.sys_io[DomainAonSel]),
    .leaf_rst_o(resets_o.rst_sys_io_n[DomainAonSel]),
    .err_o(cnsty_chk_errs[6][DomainAonSel]),
    .fsm_err_o(fsm_errs[6][DomainAonSel])
  );

  if (SecCheck) begin : gen_daon_sys_io_assert
  `ASSERT_PRIM_FSM_ERROR_TRIGGER_ALERT(
    DAonSysIoFsmCheck_A,
    u_daon_sys_io.gen_rst_chk.u_rst_chk.u_state_regs,
    alert_tx_o[0])
  end
  assign resets_o.rst_sys_io_n[Domain0Sel] = '0;
  assign cnsty_chk_errs[6][Domain0Sel] = '0;
  assign fsm_errs[6][Domain0Sel] = '0;
  assign rst_en_o.sys_io[Domain0Sel] = MuBi4True;
