...
|                       |          |            |             |               |               | this correctly).                                                                      |
| ROT_CREATOR_IDENTITY  |    no    |     no     |     yes     | yes (Digest)  |   yes (CSR)   | Software managed partition for the RoT Creator identity.                              |
| ROT_OWNER_AUTH_SLOT0  |    no    |     no     |     yes     | yes (Digest)  |   yes (CSR)   | Software managed partition for the first RoT Owner (a.k.a. RoT Creator) key manifest. |
| ROT_OWNER_AUTH_SLOT1  |    no    |     no     |     yes     | yes (Digest)  |   yes (CSR)   | Software managed RoT Owner slot 1 partition.                                          |
...
|     SOC_FUSES_CP      |    no    |     no     |     yes     | yes (Digest)  |   yes (CSR)   | SoC fuses partition for the Chip Probe (CP) manufacturing stage.                      |
|     SOC_FUSES_FT      |    no    |     no     |     yes     | yes (Digest)  |   yes (CSR)   | SoC fuses partition for the Factory Testing (FT) manufacturing stage.                 |
|        HW_CFG0        |    no    |    yes     |     yes     | yes (Digest)  |      no       | Hardware configuration partition 0.                                                   |
|                       |          |            |             |               |               | This contains a device identifier.                                                    |
|        HW_CFG1        |    no    |    yes     |     yes     | yes (Digest)  |      no       | Hardware configuration partition 1.                                                   |
|                       |          |            |             |               |               | This contains several hardware feature switches.                                      |
|        HW_CFG2        |    no    |    yes     |     yes     | yes (Digest)  |      no       | Hardware configuration partition 2.                                                   |
...
|                       |          |            |             |               |               | is not readable nor writable via the DAI. Only the LC controller                      |
