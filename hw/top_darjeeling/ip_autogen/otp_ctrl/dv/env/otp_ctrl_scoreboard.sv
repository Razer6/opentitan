...
class otp_ctrl_scoreboard #(type CFG_T = otp_ctrl_env_cfg)
  ...
  virtual task process_wipe_mem();
    forever begin
      @(posedge cfg.backdoor_clear_mem) begin
        ...
        // secret partitions have been scrambled before writing to OTP.
        // here calculate the pre-scrambled raw data when clearing internal OTP to all 0s.
        data = descramble_data(0, Secret4Idx);
        for (int i = Secret4Offset / TL_SIZE;
             i <= Secret4DigestOffset / TL_SIZE - 1;
             i++) begin
          otp_a[i] = ((i - Secret4Offset / TL_SIZE) % 2) ?
              data[SCRAMBLE_DATA_SIZE-1:TL_DW] : data[TL_DW-1:0];
        end
    ...
    case (csr_name)
      ...
      "err_code_24": begin
        ...
        if (cfg.en_cov && do_read_check && data_phase_read) begin
          bit [TL_DW-1:0] dai_addr = `gmv(ral.direct_access_address) >> 2 << 2;
          int access_part_idx = get_part_index(dai_addr);
          cov.collect_err_code_cov(24, item.d_data, access_part_idx);
        end
      end
      "err_code_25": begin
        if (cfg.m_lc_prog_pull_agent_cfg.vif.req) do_read_check = 0;
        if (cfg.en_cov && do_read_check && data_phase_read) begin
          bit [TL_DW-1:0] dai_addr = `gmv(ral.direct_access_address) >> 2 << 2;
          int access_part_idx = get_part_index(dai_addr);
          cov.collect_err_code_cov(25, item.d_data, access_part_idx);
        end
      end
      "err_code_26": begin
        if (cfg.m_lc_prog_pull_agent_cfg.vif.req) do_read_check = 0;
        if (cfg.en_cov && do_read_check && data_phase_read) begin
          bit [TL_DW-1:0] dai_addr = `gmv(ral.direct_access_address) >> 2 << 2;
          int access_part_idx = get_part_index(dai_addr);
          cov.collect_err_code_cov(26, item.d_data, access_part_idx);
        end
      end
      "err_code_27": begin
        if (cfg.m_lc_prog_pull_agent_cfg.vif.req) do_read_check = 0;
      ...
      "secret3_digest_0", "secret3_digest_1",
      "secret4_digest_0", "secret4_digest_1": begin
  ...
  virtual function void predict_digest_csrs();
    ...
    void'(ral.secret4_digest[0].predict(
          .value(otp_a[PART_OTP_DIGEST_ADDRS[Secret4Idx]]),
          .kind(UVM_PREDICT_DIRECT)));
    void'(ral.secret4_digest[1].predict(
          .value(otp_a[PART_OTP_DIGEST_ADDRS[Secret4Idx] + 1]),
          .kind(UVM_PREDICT_DIRECT)));
  endfunction
  ...
  function void cal_digest_val(int part_idx);
    ...
    case (part_idx)
      ...
      Secret4Idx: mem_q = otp_a[Secret4Offset / TL_SIZE : Secret4DigestOffset / TL_SIZE - 1];
      default: begin
  ...
  virtual function bit [TL_DW*2-1:0] get_digest_reg_val(int part_idx);
    ...
    case (part_idx)
      ...
      Secret4Idx: begin
        digest = {`gmv(ral.secret4_digest[1]),
                  `gmv(ral.secret4_digest[0])};
      end
  ...
  virtual function bit is_tl_mem_access_allowed(input tl_seq_item item, input string ral_name,
    ...
    if (addr inside {
      ...
      if (`gmv(ral.rot_creator_identity_read_lock) == 0 ||
          cfg.otp_ctrl_vif.under_error_states()) begin
        if (addr inside {
            [cfg.ral_models[ral_name].mem_ranges[0].start_addr + RotCreatorIdentityOffset :
             cfg.ral_models[ral_name].mem_ranges[0].start_addr + RotCreatorIdentityOffset +
             RotCreatorIdentitySize - 1]}) begin
          predict_err(OtpRotCreatorIdentityErrIdx, OtpAccessError);
          custom_err = 1;
          if (cfg.en_cov) begin
      ...
      if (`gmv(ral.soc_fuses_cp_read_lock) == 0 ||
          cfg.otp_ctrl_vif.under_error_states()) begin
        if (addr inside {
            [cfg.ral_models[ral_name].mem_ranges[0].start_addr + SocFusesCpOffset :
             cfg.ral_models[ral_name].mem_ranges[0].start_addr + SocFusesCpOffset +
             SocFusesCpSize - 1]}) begin
          predict_err(OtpSocFusesCpErrIdx, OtpAccessError);
          custom_err = 1;
          if (cfg.en_cov) begin
      ...
      if (`gmv(ral.soc_fuses_ft_read_lock) == 0 ||
          cfg.otp_ctrl_vif.under_error_states()) begin
        if (addr inside {
            [cfg.ral_models[ral_name].mem_ranges[0].start_addr + SocFusesFtOffset :
             cfg.ral_models[ral_name].mem_ranges[0].start_addr + SocFusesFtOffset +
             SocFusesFtSize - 1]}) begin
          predict_err(OtpSocFusesFtErrIdx, OtpAccessError);
          custom_err = 1;
          if (cfg.en_cov) begin
