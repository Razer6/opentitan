...
`define DRIVE_CHIP_TL_HOST_IF(tl_name, inst_name, sig_name) \
     force ``tl_name``_tl_if.d2h = dut.top_pwc.u_``inst_name``.``sig_name``_i; \
     force dut.top_pwc.u_``inst_name``.``sig_name``_o = ``tl_name``_tl_if.h2d; \
     force dut.top_pwc.u_``inst_name``.clk_i = 0; \
...
`define DRIVE_CHIP_TL_DEVICE_IF(tl_name, inst_name, sig_name) \
     force ``tl_name``_tl_if.h2d = dut.top_pwc.u_``inst_name``.``sig_name``_i; \
     force dut.top_pwc.u_``inst_name``.``sig_name``_o = ``tl_name``_tl_if.d2h; \
     force dut.top_pwc.u_``inst_name``.clk_i = 0; \
     uvm_config_db#(virtual tl_if)::set(null, $sformatf("*env.%0s_agent", `"tl_name`"), "vif", \
                                        ``tl_name``_tl_if);
...
tl_if racl_ctrl_pwc_tl_if(clk_ext_main, rst_n);
tl_if ac_range_check_pwc_tl_if(clk_ext_main, rst_n);
...
`ifndef GATE_LEVEL
    ...
    `DRIVE_CHIP_TL_DEVICE_IF(racl_ctrl_pwc, racl_ctrl_pwc, tl)
    `DRIVE_CHIP_TL_DEVICE_IF(ac_range_check_pwc, ac_range_check_pwc, tl)
`endif
