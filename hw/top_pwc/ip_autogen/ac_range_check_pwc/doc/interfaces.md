...
<!-- BEGIN CMDGEN util/regtool.py --interfaces ./hw/top_pwc/ip_autogen/ac_range_check_pwc/data/ac_range_check_pwc.hjson -->
Referring to the [Comportable guideline for peripheral device functionality](https://opentitan.org/book/doc/contributing/hw/comportability), the module **`ac_range_check_pwc`** has the following hardware interfaces defined
- Primary Clock: **`clk_i`**
- Other Clocks: *none*
...
|:------------------------------------|:----------------------------------------------------------------------------------------|
| AC_RANGE_CHECK_PWC.BUS.INTEGRITY    | End-to-end bus integrity scheme.                                                        |
| AC_RANGE_CHECK_PWC.CTRL.REGWEN_MUBI | Registers are protected from writes with a multi-bit encoded signal.                    |
| AC_RANGE_CHECK_PWC.CTRL.MUBI        | Registers have multi-bit encoded fields.                                                |
| AC_RANGE_CHECK_PWC.INTERSIG.MUBI    | Range overwrite singal to bypass the matching logic.                                    |
| AC_RANGE_CHECK_PWC.CTR.REDUN        | The log counter is protected by a second counter that counts in the opposite direction. |
