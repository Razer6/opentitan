...
class gpio_pwc_stress_all_vseq extends gpio_pwc_base_vseq;
  `uvm_object_utils(gpio_pwc_stress_all_vseq)

  `uvm_object_new

  task body();
    string seq_names[] = {"gpio_pwc_smoke_vseq",
                          // "gpio_pwc_common_vseq",
                          // does not support intr_test as plus_arg disable do_clear_all_interrupts
                          "gpio_pwc_random_dout_din_vseq",
                          "gpio_pwc_dout_din_regs_random_rw_vseq",
                          "gpio_pwc_intr_rand_pgm_vseq",
                          "gpio_pwc_rand_intr_trigger_vseq",
                          "gpio_pwc_random_long_reg_writes_reg_reads_vseq",
                          "gpio_pwc_rand_straps_vseq"};
    for (int i = 1; i <= num_trans; i++) begin
      uvm_sequence   seq;
      gpio_pwc_base_vseq gpio_vseq;
