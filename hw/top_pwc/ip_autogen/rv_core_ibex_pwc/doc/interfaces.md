...
<!-- BEGIN CMDGEN util/regtool.py --interfaces ./hw/top_pwc/ip_autogen/rv_core_ibex_pwc/data/rv_core_ibex_pwc.hjson -->
Referring to the [Comportable guideline for peripheral device functionality](https://opentitan.org/book/doc/contributing/hw/comportability), the module **`rv_core_ibex_pwc`** has the following hardware interfaces defined
- Primary Clock: **`clk_i`**
...
|:------------------------------------------------|:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| RV_CORE_IBEX_PWC.BUS.INTEGRITY                  | End-to-end bus integrity scheme.                                                                                                                                                                 |
| RV_CORE_IBEX_PWC.SCRAMBLE.KEY.SIDELOAD          | The scrambling key for the icache is sideloaded from OTP and thus unreadable by SW.                                                                                                              |
| RV_CORE_IBEX_PWC.CORE.DATA_REG_SW.SCA           | Data independent timing.                                                                                                                                                                         |
| RV_CORE_IBEX_PWC.PC.CTRL_FLOW.CONSISTENCY       | Correct PC increment check.                                                                                                                                                                      |
| RV_CORE_IBEX_PWC.CTRL_FLOW.UNPREDICTABLE        | Randomized dummy instruction insertion.                                                                                                                                                          |
| RV_CORE_IBEX_PWC.DATA_REG_SW.INTEGRITY          | Register file integrity checking. Note that whilst the core itself is duplicated (see LOGIC.SHADOW) the register file is not. Protection is provided by an ECC.                                  |
| RV_CORE_IBEX_PWC.DATA_REG_SW.GLITCH_DETECT      | This countermeasure augments DATA_REG_SW.INTEGRITY and checks for spurious write-enable signals on the register file by monitoring the one-hot0 property of the individual write-enable strobes. |
| RV_CORE_IBEX_PWC.LOGIC.SHADOW                   | Shadow core run in lockstep to crosscheck CPU behaviour. This provides broad protection for all assets with the Ibex core.                                                                       |
| RV_CORE_IBEX_PWC.FETCH.CTRL.LC_GATED            | Fetch enable so core execution can be halted.                                                                                                                                                    |
| RV_CORE_IBEX_PWC.EXCEPTION.CTRL_FLOW.LOCAL_ESC  | A mechanism to detect and act on double faults. Local escalation shuts down the core when a double fault is seen.                                                                                |
| RV_CORE_IBEX_PWC.EXCEPTION.CTRL_FLOW.GLOBAL_ESC | A mechanism to detect and act on double faults. Global escalation sends a fatal alert when a double fault is seen.                                                                               |
| RV_CORE_IBEX_PWC.ICACHE.MEM.SCRAMBLE            | ICache memory scrambling.                                                                                                                                                                        |
| RV_CORE_IBEX_PWC.ICACHE.MEM.INTEGRITY           | ICache memory integrity checking.                                                                                                                                                                |
