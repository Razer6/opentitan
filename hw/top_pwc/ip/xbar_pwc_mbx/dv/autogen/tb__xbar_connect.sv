...
`CONNECT_TL_HOST_IF(pwc_mbx, dut, clk_mbx_i, rst_n)
...
`CONNECT_TL_DEVICE_IF(racl_ctrl_pwc, dut, clk_mbx_i, rst_n)
`CONNECT_TL_DEVICE_IF(ac_range_check_pwc, dut, clk_mbx_i, rst_n)
