...
%>\
  ...
  input logic ${lib.bitarray(p_in.bits.width(), 32)} cio_${m["name"]}_${p_in.name}_p2d_i,
    % endfor
    % for p_out in outputs + inouts:
  output logic ${lib.bitarray(p_out.bits.width(), 32)} cio_${m["name"]}_${p_out.name}_d2p_o,
  output logic ${lib.bitarray(p_out.bits.width(), 32)} cio_${m["name"]}_${p_out.name}_en_d2p_o,
    % endfor
  % endfor
