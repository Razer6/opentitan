...
#(
  ...
  localparam int unsigned TAW = vbits(TopCount)
) (
  input logic                    clk_i,
  input logic                    rst_ni,
  ...
  assign low_read_end = (free_slots_q < RomLatency + 1) && kmac_rom_rdy_i;

  always_comb begin
    free_slots_d = free_slots_q;
    if (low_read_start) free_slots_d -= 1;
    if (low_read_end) free_slots_d += 1;
  end
