...
#(
  ...
  // HW Debug lifecycle enable signal (latched version from pinmux, only used for JTAG/TAP or DMI
  // gating)
  ...
  // debug enable gating
  if (UseDmiInterface) begin : gen_dmi_typedef 
    typedef enum logic [1:0] {
      PmEnDmiTlulLcGate,
      PmEnDmiTlulAdapter,
      PmEnLastPos
    } rv_dm_pm_en_e;
  end else begin : gen_jtag_tap_typedef
  ...
  if (UseDmiInterface) begin : gen_dmi_gating_signals
    // Convert LC signal from pinmux to MuBi4 value:  As the multi-bit LC signal may cross clock
    // domains, it may not have a valid LC value in every clock cycle.  This is not to be
    // interpreted as an injected fault, so the conversion to a boolean tests for a strictly true
    // LC value and interprets all other values as false.
    assign dmi_tlul_adapter_en = prim_mubi_pkg::mubi4_bool_to_mubi(
                                  lc_tx_test_true_strict(pinmux_hw_debug_en[PmEnDmiTlulAdapter]));
  end else begin : gen_jtag_tap_gating_signals
    // SEC_CM: DM_EN.CTRL.LC_GATED
  ...
  if (UseDmiInterface) begin : gen_dmi_ndmreset_req
    assign ndmreset_req_qual = &{ndmreset_ack_rom, ndmreset_ack_dmi};
  end else begin : gen_jtag_tap_ndmreset_req
    assign ndmreset_req_qual = ndmreset_ack_rom
  end
  ...
  tlul_adapter_host #(
    ...
    //    via JTAG or DMI so that bus integrity errors can be told appart from regular bus errors.
  ...
  if (UseDmiInterface) begin : gen_dmi_gating
    ...
    assign unused_signals = scan_rst_ni;
  end else begin : gen_jtag_tap_gating
  ...
  `ifndef DMIDirectTAP
    ...
    logic tck_muxed;
    logic trst_n_muxed;
    logic dmi_req_valid_int, dmi_rsp_ready_int;
    prim_clock_mux2 #(
      .NoFpgaBufG(1'b1)
    ) u_prim_clock_mux2 (
    ...
    dmi_jtag #(
      ...
      .dmi_req_o        (dmi_req),
      .dmi_req_valid_o  (dmi_req_valid_int),
      .dmi_req_ready_i  (dmi_req_ready & dmi_en),

      .dmi_resp_i       (dmi_rsp      ),
      .dmi_resp_ready_o (dmi_rsp_ready_int),
    ...
    // Gate signals before leaving
    assign dmi_req_valid = dmi_req_valid_int & dmi_en;
    assign dmi_rsp_ready = dmi_rsp_ready_int & dmi_en;
    // Unused signals from the DMI part
    assign dmi_intg_error   = 1'b0;
    assign ndmreset_ack_dmi = 1'b0;
  `endif
  end
  ...
  // Flip-Flops //
  ////////////////
  if (UseDmiInterface) begin : gen_dmi_flops
