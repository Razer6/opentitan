...
module prim_ram_1p_scr import prim_ram_1p_pkg::*; #(
  ...
  // Unread unused bits from keystream
  if ((Width % 64) > 0) begin : gen_unread_last_piped
    localparam int UnusedWidth = 64 - (Width % 64);
    logic [UnusedWidth-1:0] unused_keystream_pipelined;
    assign unused_keystream_pipelined = keystream_pipelined[NumParScr * 64 - 1 -: UnusedWidth];
  end
