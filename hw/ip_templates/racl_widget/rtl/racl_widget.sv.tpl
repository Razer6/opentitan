...
module ${module_instance_name} import ${module_instance_name}_reg_pkg::*; #(
  ...
  parmeter int unsigned           NumSubscribingIps = 1
) (
  input  logic                                      clk_i,
  input  logic                                      rst_ni,
  ...
  output racl_policy_vec_t                          policies_o,
  // RACL violation information.
  input logic            [NumSubscribingIps-1:0]    racl_error_i,
  input racl_error_log_t [NumSubscribingIps-1:0]    racl_error_log_i
);
  ...
  ${module_instance_name}_reg_top u_racl_widget_reg (
    .clk_i                  ( clk_i                ),
    .rst_ni                 ( rst_ni               ),
...
% for policy in policies:
  assign policy_${policy['name'].lower()}.read_perm = reg2hw.policy_${policy['name'].lower()}.read_perm.q;
  assign policy_${policy['name'].lower()}.write_perm = reg2hw.policy_${policy['name'].lower()}.write_perm.q;
...
% endfor
  ...
  racl_rolt_t racl_error_role;
  assign racl_error_role = |racl_error_i.role;

  logic racl_error_role;
  assign racl_error_role = |racl_error_i.role;

  logc racl_error_write_read;
  assign racl_error_write_read = |racl_error_i.write_read;

  logic first_error;
  assign first_error = ~reg2hw.error_log.error_valid.q & racl_error;

  // Writing 1 to the error valid bit clears the log again
  logic clear_log;
  assign clear_log = reg2hw.error_log.error_valid.q & reg2hw.error_log.error_valid.qe;

  assign hw2reg.error_log.error_valid.d  = ~clear_log;
  assign hw2reg.error_log.error_valid.de = racl_error | clear_log;

  // Overflow is raised when error is valud and a new error is coming in
  assign hw2reg.error_log.overflow.d  = ~clear_log;
  assign hw2reg.error_log.overflow.de = (reg2hw.error_log.error_valid.q & racl_error) | clear_log;
  ...
  `ASSERT_PRIM_REG_WE_ONEHOT_ERROR_TRIGGER_ALERT(RegWeOnehotCheck_A, u_racl_widget_reg,
                                                 alert_tx_o[0])
endmodule
